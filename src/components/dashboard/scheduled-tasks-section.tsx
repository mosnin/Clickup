    ...
    <section>
      ...
      <ul className="mt-3 space-y-2">
        {(schedules ?? []).map((st) => (
          <li
            key={st._id}
            className="flex flex-wrap items-center gap-2 rounded-2xl panel px-4 py-2.5 text-sm"
          >
            <CalendarClock
        ...
        {schedules !== undefined && schedules.length === 0 && !creating && (
          <li className="rounded-2xl panel p-6 text-center text-sm text-muted-foreground">
            No schedules yet.
          </li>
        )}
      </ul>
...
function CreateScheduleForm({
  ...
  return (
    <form
      className="mt-3 flex flex-wrap items-end gap-3 rounded-2xl panel p-4"
      onSubmit={async (e) => {
        e.preventDefault();
