...
// A tree of things, drawn.
//
// The presentational primitive: nested branches that spring open, a highlight
// that slides to whatever the cursor is over, and an icon that swaps with a
// little rotation when a branch opens. It takes a plain array and renders it —
// no data source, no routing, no actions.
//
// The app's own sidebar does NOT use this component. It uses the three
// behaviours, which live in `dashboard/sidebar-tree-motion.tsx`: the real tree
// carries per-row create menus, overflow menus, inline creation, badges and
// active state, and a component whose contract is "give me an array" cannot
// hold any of that. Sharing the motion rather than the markup is what keeps
// both honest — this stays a primitive you can drop anywhere, and the sidebar
// stays a place you can work.
...
export type FileTreeElement = {
  ...
  /** Tints the item to mark it as newly added / relevant. */
  highlight?: boolean;
...
export type FileTreeProps = {
  ...
  className?: string;
  /** Colour for items with `highlight: true`. Defaults to the app's accent. */
  highlightColor?: string;
...
export function FileTree({
  ...
  return (
    ...
    >
      <div className={cn("overflow-hidden rounded-xl bento", className)}>
        <div
          ref={containerRef}
          className="relative isolate w-full p-2"
          onMouseLeave={() => setHighlightBounds(null)}
        >
          <FileTreeHoverHighlight className="z-0 rounded-lg bg-muted/60" />
          {elements.map((node) => (
