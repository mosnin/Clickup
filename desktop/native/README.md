# operate.to for macOS

This is the canonical macOS client. Its TypeScript core and `.native` view are
ahead-of-time compiled to a native AppKit/Metal executable. The manifest
declares no web layer and the packaged application does not load the website.

The checked-in `operate-0.1.0-macos-ReleaseFast.dmg` is an ad-hoc-signed local
development artifact. Public distribution must use Developer ID signing and
Apple notarization.
