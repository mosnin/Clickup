...
0. **Read the room first**: call \`brief\` once at session start. It returns the decisions this team has already made, the questions still open, and the limits you are operating under. Relitigating a settled question is the most expensive thing a newly arrived agent does, and this is the one call that prevents it. Do this before looking for work, not after.
...
2. **Load and acknowledge the source of truth**: \`get_task_context\` is ONE call for everything you need before touching a task — the readiness verdict, open revisions, decisions in force, every attached \`contextPacket\` at its current version, and how previous runs on it ended, in relevance order (worst-to-miss first). It hands back an \`acknowledge\` argument built from the packets it actually returned; pass that straight to \`acknowledge_task_context\`. Assembling that argument by hand from four separate reads is the step most often got subtly wrong, and getting it wrong means claiming to have read a version you have not. (\`get_task\` still returns the full record of one task; prefer \`get_task_context\` before working.) This receipt is an execution invariant: a later packet edit makes you stale, and Operate refuses claim/start/complete until you re-read and acknowledge. Treat packet objectives and constraints as authoritative and record versions in your run summary.
...
4. **Claim before working**: call \`claim_task\` after context and decision impacts are current. If it fails because the task is taken, move on. Claims expire after 60 minutes, so re-claim if a long task runs past that. On a list whose \`claimPolicy\` is \`required\` the claim is not advisory: every write to the task is refused until you hold one, and the task read tells you which kind of list you are on.
...
8. **Respect dependencies and gates**: \`get_task\` shows \`blockedByTaskIds\` and \`requiresApproval\`. Completing is refused while a blocker is open. If a task needs approval, finish the work, tick the checklist, and call \`complete_task\` with a \`note\` describing what you did. It does NOT fail on a gated task: your completion is recorded and it returns \`pending: true\`. The task is not complete — do not call it again, and do not wait for it. Pick up other work; you are notified only if a human sends it back. (\`request_approval\` is still there for raising a gate or asking a human to look before you are finished; it is no longer the way to hand finished work over.) Report artifacts (PR links, docs) and cost via \`finish_run\`.
