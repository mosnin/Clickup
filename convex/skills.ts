...
3. **Load the source of truth**: \`get_task\` returns every attached \`contextPacket\` in full. Read those before acting; treat their objective, constraints, and decisions as authoritative. Record the packet versions in your run summary. If a packet changes while you work, re-read it before finishing instead of relying on stale context.
