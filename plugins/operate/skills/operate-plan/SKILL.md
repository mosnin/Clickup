...
Operate's hierarchy is Workspace → Space → Project → List → Task → Subtask. A List may sit directly in a Space when it does not need a Project. A roadmap sequences Projects into phases; it is not another container. Every `projects` input entry represents one durable workstream and materializes as a Project containing its Lists inside the chosen Space.
...
3. Design durable workstreams. Put each workstream in one `projects` entry so it materializes as a Project with its Lists. Give every task a unique ref, concrete acceptance checklist, required capabilities, and only genuine dependency edges.
...
6. Read the returned manifest and report the Space, roadmap, workstream Projects and Lists, tasks, assumptions, open questions, and `reviewStatus` to the user.
