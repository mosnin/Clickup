...
1. Call `whoami` and `list_wake_inbox` at session start so an offline interval or lost push cannot hide work. Treat each `task.assigned`, `task.ready`, or `mention.created` wake as a prompt, not as complete context. Call `acknowledge_wake` with its delivery id; for task wakes call `next_task`, then `get_task`. For mention wakes, read the relevant task or collaboration feed before acting.
