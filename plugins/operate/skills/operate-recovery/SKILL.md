...
3. Check durable wake delivery status and attempts, `acknowledgedAt`, `contextDrifted`, the dispatch and current context fingerprints, estimated token load, claim freshness, agent status, required capabilities, concurrency, dependencies, approval gates, and evidence/error fields. A failed wake means the runtime must recover it from `list_wake_inbox` or its endpoint needs repair; `poll_required` means the wake is waiting in that inbox; a delivered but unacknowledged wake means the endpoint accepted it but agent consumption is unproven; context drift requires re-reading and acknowledgement. None of these conditions by itself justifies duplicating a fresh active attempt.
