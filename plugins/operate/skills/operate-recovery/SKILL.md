...
3. Check durable wake delivery status and attempts, `contextDrifted`, the dispatch and current context fingerprints, estimated token load, claim freshness, agent status, required capabilities, concurrency, dependencies, approval gates, and evidence/error fields. A failed wake means the runtime must poll or its endpoint needs repair; context drift requires re-reading and acknowledgement. Neither condition by itself justifies duplicating a fresh active attempt.
