...
- **Components as data (`src/lib/ui-components.ts` + `convex/uiComponents.ts` + `user-component.tsx`).** The ceiling this removes: panels were a hardcoded registry, so "customising the UI" could only mean choosing which of nine to show and in what order — you could not author a tenth. A component is now a **definition** (source + filter + shape + fields + grouping + limit + density) stored in `uiComponents`, and ONE generic renderer draws any of them, so adding a kind of panel needs no deploy. Two people in one workspace can end up looking at applications that don't resemble each other over identical primitives. **The vocabulary is closed and that is a security boundary, not a style rule** — agents author these, so `normalizeComponent` drops any source/shape/field/group/sort it doesn't enumerate, and `uiComponents.resolve` has exactly one branch per value the normalizer emits. Nothing is a script, an expression, or a path into the database. Rows are access-checked at the scope AND re-checked on the way out: a panel is a query someone else may have written and must never read across a boundary its author couldn't cross. Layout ids namespace as `custom:<id>` (`componentWidgetId`), so built-ins and authored panels sit in one registry and one tray. The builder's preview is the panel itself running the same query — not a mockup, so there is no gap for it to lie in. See `tests/ui-components.test.ts`.
