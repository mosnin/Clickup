...
- **Pace Step 5 (current):** Mobile is the product. New `<BottomTabs />` (Home / Inbox / + / Brain / Trash, the `+` opens the command palette) mounted in the dashboard layout, hidden on `md+`. Center button gets a haptic tap on press; nav links also tap. New `lib/haptics.ts` wraps `@capacitor/haptics` so iOS/Android wrappers vibrate while web no-ops. New `lib/use-swipeable.ts` is a touch-only horizontal swipe hook with a vertical-direction lock (so page scroll still works). New `MobileTaskList` (block sm:hidden) replaces the table on phones with swipeable cards — swipe right completes (haptic success), swipe left deletes with the existing Undo toast (haptic medium tap). Calendar view splits into `MobileSchedule` (date-grouped vertical list with overdue / today highlighting, mobile only) plus the existing month grid (sm+). Gantt splits into `MobileGantt` (linear schedule with status colors) plus the existing horizontal timeline (sm+). Dashboard layout adds `pb-20` mobile padding so content clears the bottom bar.
