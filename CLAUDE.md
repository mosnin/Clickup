...
- **Pace Step 4:** First-run experience. `users.completeOnboarding` + `users.personalSpaceId` queries. Onboarding rewritten as a 3-step wizard (Workspace → Invite → Starter template) with skippable steps and a step indicator. Solo users skip step 2 entirely. After Finish: applies the chosen template to the personal space, sends invites in parallel, marks `users.onboardedAt`, and routes to the new workspace (or `/dashboard`). New `<FirstRunDialog />` mounted in the dashboard layout shows once per user-per-device after onboarding completes — surfaces ⌘K, `?`, click-to-edit, and the 30-day Trash window. Persisted dismissal in `localStorage` under `pace.first-run.dismissed`.
- **Pace Step 5:** Mobile is the product. New `<BottomTabs />` (Home / Inbox / + / Brain / Trash, the `+` opens the command palette) mounted in the dashboard layout, hidden on `md+`. Center button gets a haptic tap on press; nav links also tap. New `lib/haptics.ts` wraps `@capacitor/haptics` so iOS/Android wrappers vibrate while web no-ops. New `lib/use-swipeable.ts` is a touch-only horizontal swipe hook with a vertical-direction lock (so page scroll still works). New `MobileTaskList` (block sm:hidden) replaces the table on phones with swipeable cards — swipe right completes (haptic success), swipe left deletes with the existing Undo toast (haptic medium tap). Calendar view splits into `MobileSchedule` (date-grouped vertical list with overdue / today highlighting, mobile only) plus the existing month grid (sm+). Gantt splits into `MobileGantt` (linear schedule with status colors) plus the existing horizontal timeline (sm+). Dashboard layout adds `pb-20` mobile padding so content clears the bottom bar.
- **Pace Step 6:** Realtime made visible. New `presence` table; `convex/presence.ts` exposes `heartbeat` (10s, tri-state typing arg so the page-level heartbeat can run alongside the composer pulse without racing), `clear` (fired on `pagehide` / unmount), `listForFocus` (filters within a 30s freshness window), and an `internal.presence.sweepStale` cron at a 5-minute interval. New `lib/use-presence.ts` hook drives the heartbeat + viewers; `<PresenceStack />` renders an avatar stack (up to 3 + `+N` overflow, pulsing dot when typing). Mounted on task-detail, doc-editor, and list-page headers. Comments composer pulses `typing=true` on input (debounced false after 2.5s idle) and the Comments root surfaces "X is typing…" from `listForFocus`. New `lib/use-recently-changed.ts` diffs successive query results and returns a Set of recently-changed ids; list-view (desktop `<tr>`) and mobile-task-list (card) flash `bg-brand-50` for ~1.6s when a teammate changes status / completedAt / title / due / priority.
- **Pace Step 7:** CRDT docs. Tiptap docs are now backed by a Yjs CRDT, synced through Convex. Schema: `docs.snapshot` (bytes) + `docs.snapshotSequence` (the seq through which the snapshot incorporates updates), and a new `docUpdates` table indexed `by_doc_seq` storing each Yjs binary update. New `convex/docUpdates.ts` with `append` (insert-with-next-sequence; OCC handles concurrent inserts; auto-schedules compaction at 60 pending updates) and `listSinceSequence`. Compaction lives in `convex/yjs.ts` (Node runtime): an internalAction loads snapshot + pending updates, merges via `Y.applyUpdate` + `Y.encodeStateAsUpdate`, then atomically writes the new snapshot and prunes the folded rows. New `src/lib/use-yjs-doc.ts` is the client sync provider: applies the snapshot on mount, subscribes to `listSinceSequence` for incoming updates (origin = `"convex"` to suppress echo), and forwards local Y.Doc updates back via `docUpdates.append`. Doc editor swaps `content` for `Collaboration.configure({ document: ydoc })`, disables StarterKit history (Yjs owns undo), and renders a "Synced" / "Loading" badge in the toolbar. Legacy `docs.content` is kept as a one-shot seed for older rows. `docs.purge` cascades the docUpdates rows; `docs.updateContent` is gone (replaced entirely by `docUpdates.append`). Doc route bundle grew from 97 kB → 139 kB to absorb yjs + y-prosemirror + the Tiptap collab extension. Live cursors are a follow-up — they need a separate awareness transport (`y-protocols/awareness`).
- **Pace Step 8:** AI with agency. New `ai.quickTask` action uses OpenAI function calling (`tool_choice: create_task`) to convert a sentence into a task with the right list, priority, due offset, and assignees inferred. Available list/member ids are passed to the model as enums on the tool definition so it can't hallucinate IDs. Internal `_quickTaskContext` query gathers lists + members in scope (personal or workspace); internal `_createTaskFromQuick` mutation re-checks scope membership before insert. The command palette gains a pinned bottom row "Create task: <query>" once the input is 3+ chars: it picks the scope from the URL (`/dashboard/w/[id]/...` → workspace, else personal) and the current list (`/dashboard/l/[id]/...`), runs the action, navigates to the new task, and shows an Undo toast wired to `tasks.remove`. `ai.brainSearch` now also accepts optional `currentTaskId` / `currentListId` / `currentDocId`; when provided, an internal `_brainPageContext` query resolves them to a short "you're looking at this" prefix on the user prompt. Brain page reads `?taskId=...` / `?listId=...` / `?docId=...` from the URL and forwards them, with a pill above the form showing the resolved subject.
- **Pace Step 9:** Filters + bulk actions. New `src/lib/use-task-filter.ts` exposes a per-list filter (`mine` / `dueThisWeek` / `highPriorityPlus` / `hideCompleted`) persisted in localStorage under `pace.filter.<listId>`. `useFilteredTasks(tasks, filter, statuses, meClerkId)` runs filtering in a `useMemo`. Hide-completed defaults ON; the chip flips to "show completed". New `<FilterChips />` (chip bar) is rendered above List + Board views — Calendar / Gantt are time-anchored so filters would be more confusing than useful. List view gains a bulk-select mode toggled by a "Select" button on the chip bar: a checkbox column appears, and selecting any row pops `<BulkActionBar />` at the bottom with three actions: Complete (sets every selection to the list's first complete-category status), Assign to me (adds the current user's clerk id to each task's assignees), and Delete (soft-deletes all selected with a single Undo toast that restores the batch). Bulk actions are client-side fan-outs of the existing single-task mutations — fine at the sizes we target.
- **Pace Step 10:** Time depth. New `/dashboard/time` route is a Mon→Sun timesheet for the calling user, with the active week encoded as `?week=YYYY-MM-DD` (Monday) so URLs are bookmarkable. Day totals + week + billable totals across the top, then per-day sections with editable rows. New `timeEntries.listForUserInRange({from, to})` Convex query is a range scan on the `by_user_started` index returning entries with the task title + listId resolved server-side; `timeEntries.manualCreate` inserts a completed entry directly (both bounds required, duration computed server-side); `timeEntries.update` extended to accept `startedAt` / `endedAt` and recompute `durationMs`. The "Add entry" form picks a task via `api.search.palette` filtered to `kind === "task"`, defaulting to today's 9–10am. Sidebar gains a "Time" link below Inbox using the `Clock` icon. Two-way Slack (slash commands, signed webhooks) is deferred to a later step — needs OAuth + a real Slack workspace to validate.
- **Jobs audit fixes (current):** A brutal product/design audit prompted four parallel cleanup tracks. (1) Brand finally tells the truth: `--color-brand-*` flipped from indigo to emerald (`#10b981` / `#059669` / `#047857`); new `--color-accent-*` mapped to amber so `bg-amber-*` ad-hoc usages have a token to migrate to; `next/font/google` Geist wired in `src/app/layout.tsx` with weights 400/500/600/700 — the product was rendering OS sans-serif before this. (2) The Pace moment surfaces on `/dashboard` — Quick Task hero is the first thing a user sees ("What needs doing?" + a wide pill input that calls `api.ai.quickTask`); personal/workspace cards demoted to a secondary section below. (3) Marketing copy stops dancing: tagline is **"Type. Done."** with sub-tagline **"Pace turns a plain-English sentence into the right task on the right list. One keystroke."** Landing page rebuilt around a Quick Task demo block + three concrete claims (one keystroke / never the wrong list / stays out of your way). Features page is four pillars, not a soup. Pricing collapsed from three tiers to two — Free + Pro at $10/user/month. (4) Dead-weight nav entries hidden: Whiteboards removed from sidebar (and "+ Whiteboard" gone) because tldraw still ships its watermark; Chat + Reports tabs spliced from the workspace tabs array. Routes (`/dashboard/wb/[id]`, `?tab=chat`, `?tab=reports`) stay live — direct links + soft-delete restore still work — but the entry points are gone. STYLESHEET.md's cleanup queue is now resolved on items 1, 2, 3.
